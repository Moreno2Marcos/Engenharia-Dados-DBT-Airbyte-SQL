...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/ena/chave/fintech-494320-d8e2f9ce635d.json"

# Configure o projeto e dataset
project_id = "fintech-494320"
dataset_id = "fintechstaging"

# Diretório onde os arquivos CSV estão salvos
csv_directory = "/ena/dados/dados"

# Inicializa o cliente do BigQuery
client = bigquery.Client(project=project_id)

# Função para criar o dataset (banco de dados) se ele não existir
def fintech_cria_dataset(dataset_id):
...
def fintech_carrega_bigquery(table_name, csv_file_path):
...
fintech_cria_dataset(dataset_id)

# Arquivos CSV e nomes das tabelas correspondentes
csv_files = {
    "stg_clientes": os.path.join(csv_directory, "stg_clientes_fintech.csv"),
    "stg_data": os.path.join(csv_directory, "stg_data_fintech.csv"),
    "stg_localidades": os.path.join(csv_directory, "stg_localidades_fintech.csv"),
    "stg_produtos": os.path.join(csv_directory, "stg_produtos_fintech.csv"),
    "stg_transacoes": os.path.join(csv_directory, "stg_transacoes_fintech.csv")
}

# Carrega cada CSV no BigQuery
for table_name, csv_file_path in csv_files.items():
    fintech_carrega_bigquery(table_name, csv_file_path)
